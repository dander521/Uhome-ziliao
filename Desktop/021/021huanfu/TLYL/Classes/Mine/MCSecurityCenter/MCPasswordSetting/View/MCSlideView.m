...
#define Selected_Color   RGB(54, 128, 221)

@interface MCSlideView ()
...
//            CGFloat WIDTH_ = [MCSlideView getWidthWithTitle:self.selectedTitleArray[i] font:15];
//            CGFloat LEFT = (btn.frame.size.width-WIDTH_)/2.0;
            
            UIView *view1 = [[UIView alloc] initWithFrame:CGRectMake(0, btn.frame.size.height-SLIDEHEIGHT,G_SCREENWIDTH/2.0, SLIDEHEIGHT)];
...
//        lineView.backgroundColor = RGB(239, 246, 252);
//        lineView.frame = CGRectMake(G_SCREENWIDTH / 2-0.5, 2, 1, TOPHEIGHT - 4);
...
-(void)createUICollectionView
{
    ...
    [_collectionView registerClass:[MCLoginPasswordCollectionViewCell class]  forCellWithReuseIdentifier:NSStringFromClass([MCLoginPasswordCollectionViewCell class]) ];
    
    [_collectionView registerClass:[MCPayPasswordCollectionViewCell class] forCellWithReuseIdentifier:NSStringFromClass([MCPayPasswordCollectionViewCell class])];
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row==0)
    {
        MCPayPasswordCollectionViewCell *cell=[collectionView dequeueReusableCellWithReuseIdentifier:NSStringFromClass([MCPayPasswordCollectionViewCell class]) forIndexPath:indexPath];
        
        //点击修改资金密码
        cell.block=^(NSDictionary *dic){
            if ([wself.delegate respondsToSelector:@selector(modifyPayPwdWithDictionary:)]) {
                [wself.delegate modifyPayPwdWithDictionary:dic];
            }
        };
        ...
        MCLoginPasswordCollectionViewCell *cell=[collectionView dequeueReusableCellWithReuseIdentifier:NSStringFromClass([MCLoginPasswordCollectionViewCell class]) forIndexPath:indexPath];
        //点击修改登录密码
        cell.block=^(NSDictionary *dic){
            if ([wself.delegate respondsToSelector:@selector(modifyLoginPwdWithDictionary:)]) {
                [wself.delegate modifyLoginPwdWithDictionary:dic];
            }
        };
