//
//  MCQiPaiXiaJiReportTableViewCell.m
//  TLYL
//
...
@interface MCQiPaiXiaJiReportTableViewCell ()
...
@property (nonatomic,strong)UILabel * lookZhanDui;

@end

@implementation MCQiPaiXiaJiReportTableViewCell
...
- (void)initView{
    ...
    UILabel * zhongJiangMoney=[self GetAdaptiveLable:CGRectMake((G_SCREENWIDTH-60)/2.0+L, 10, W, H) AndText:@"加载中" andFont:font andTextColor:RGB(102,102,102) andTextAlignment:NSTextAlignmentLeft];
    ...
    UILabel * duiZhanMoney=[self GetAdaptiveLable:CGRectMake((G_SCREENWIDTH-60)/2.0+L, 10+H, W, H) AndText:@"加载中" andFont:font andTextColor:RGB(249,84,83) andTextAlignment:NSTextAlignmentLeft];
...
-(void)setDataSource:(MCQiPaiXiaJiReportCommModel *)dataSource{
    ...
    //查看战队：战队
    if ([dataSource.ChildNum intValue]>0) {
        _lookZhanDuiLab.text=@"战队";
        _lookZhanDuiLab.hidden=NO;
        _lookZhanDui.hidden=NO;
    }else{
        _lookZhanDuiLab.hidden=YES;
        _lookZhanDui.hidden=YES;
    }
    
    _timeLab.text = dataSource.UserName;
}
