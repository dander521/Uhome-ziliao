...
@property (nonatomic, weak) MGBaseSwipeTableViewCell * currentCell;
@end
...
@property (nonatomic, weak) MGBaseSwipeTableViewCell * cell;
...
#pragma mark MGBaseSwipeTableViewCell Implementation

@interface MGBaseSwipeTableViewCell ()
@end
@implementation MGBaseSwipeTableViewCell
{
...
- (NSMutableArray *)getRangeStr:(NSString *)text findText:(NSString *)findText{
    ...
    if (rang.location != NSNotFound && rang.length != 0){
        ...
        model.range=rang;
        [arrayRanges addObject:model];//将第一次的加入到数组中
        NSRange rang1 = {0,0};
        ...
        for (int i = 0;; i++){
            ...
            RangeModel * model1 =[[RangeModel alloc]init];
            model1.range=rang1;
            if (rang1.location == NSNotFound && rang1.length == 0){
                break;
            }else//添加符合条件的location进数组
                [arrayRanges addObject:model1];
        }
        return arrayRanges;
    }
    return nil;
...
-(void) expandSwipe: (MGSwipeDirection) direction animated: (BOOL) animated
{
    ...
    if(!_activeExpansion && expSetting.fillOnTrigger) {
        ...
        if (buttonsView) {
            ...
            __weak MGBaseSwipeTableViewCell * weakself = self;
...
-(void) panHandler: (UIPanGestureRecognizer *)gesture
{
    ...
    if (gesture.state == UIGestureRecognizerStateBegan) {
        ...
        if (!_allowsMultipleSwipe) {
            ...
            for (MGBaseSwipeTableViewCell * cell in cells) {
                if ([cell isKindOfClass:[MGBaseSwipeTableViewCell class]] && cell != self) {
