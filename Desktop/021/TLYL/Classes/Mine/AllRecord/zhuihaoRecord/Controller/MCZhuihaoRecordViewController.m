//
//  MCZhuihaoRecordViewController.m
//  TLYL
//
...
@interface MCZhuihaoRecordViewController ()
<
UITableViewDelegate,
...
@property(nonatomic, strong)MCUserChaseRecordModel * userChaseRecordModel;
...
@implementation MCZhuihaoRecordViewController
...
-(void)setProperty{
    ...
    if (!_LotteryCode) {
        _LotteryCode=@"";//    LotteryCode	是	Int /String	彩种编码 / 查全部时传 空字符串
    }
    ...
    self.start_minDate = [MCRecordTool getLaterDateFromDate:[NSDate date] withYear:-1 month:-1 day:-3];
                ...
                [weakself.datePicker removeFromSuperview];
                    ...
                    weakself.viewPop.cellDidSelected = ^(NSInteger i) {
                        ...
                        if (i == 0) {
                            ...
                            weakself.start_minDate = [MCRecordTool getLaterDateFromDate:[NSDate date] withYear:-1 month:-1 day:-3];
...
-(void)loadData:(Compeletion)compeletion{
    ...
    MCUserChaseRecordModel * userChaseRecordModel = [[MCUserChaseRecordModel alloc]initWithDic:dic];
    [userChaseRecordModel refreashDataAndShow];
    self.userChaseRecordModel = userChaseRecordModel;
    
    userChaseRecordModel.callBackFailedBlock = ^(ApiBaseManager *manager, NSString *errorCode) {
        
        compeletion(NO,nil);
        
    };
    
    userChaseRecordModel.callBackSuccessBlock = ^(id manager) {
...
-(void)setData:(NSDictionary *)dic{
    ...
    for (NSDictionary * temp in arr) {
        MCUserChaseRecordDataModel * model = [MCUserChaseRecordDataModel mj_objectWithKeyValues:temp];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    return [MCZhuihaoRecordTableViewCell computeHeight:nil];
}


- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    
    NSString *reuseIdentifier =[NSString stringWithFormat:@"MCZhuihaoRecordTableViewCell-%ld-%ld",(long)indexPath.section,(long)indexPath.row];
    MCZhuihaoRecordTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:reuseIdentifier];
    if (!cell) {
        cell = [[MCZhuihaoRecordTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:reuseIdentifier];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    MCMCZhuihaoRecordDetailViewController * vc=[[MCMCZhuihaoRecordDetailViewController alloc]init];
    vc.IsHistory=_IsHistory;
    vc.model=_dataMarray[indexPath.row];
...
//#pragma mark-彩种选择
//            if (type==0) {
//                MCSignalPickView * pickView=[MCSignalPickView InitWithTitle:@"选择彩种"];
//                pickView.dataSource=arrTemp;
//                [pickView show];
//                pickView.block = ^(NSString *selectedType) {
//                    weakself.popView.label1.text=selectedType;
//                    for (NSDictionary *dic in arr) {
//                        if ([dic[@"name"] isEqualToString:selectedType]) {
//                            weakself.LotteryCode=dic[@"lotteryId"];
//                        }
//                    }
//                };
...
//                MCSignalPickView * pickView=[MCSignalPickView InitWithTitle:@"选择彩种"];
//                pickView.dataSource=@[@"当前记录",@"历史记录"];
//                [pickView show];
//                pickView.block = ^(NSString *selectedType) {
//
//                    weakself.popView.label2.text=selectedType;
//
//                    if ([selectedType isEqualToString:@"历史记录"]) {
//                        weakself.IsHistory = YES;
...
//
//                        if ([MCRecordTool bissextile:(int)[comps year]]) {
...
//
//                        weakself.start_minDate = [NSDate dateWithTimeIntervalSinceNow:- ([mo intValue ]+ 3) * 3600 * 24];
//                        weakself.start_maxDate = [NSDate dateWithTimeIntervalSinceNow:-4 * 3600 * 24];
//
//                        weakself.end_minDate = [NSDate dateWithTimeIntervalSinceNow:- ([mo intValue ]+ 3) * 3600 * 24];
//                        weakself.end_maxDate = [NSDate dateWithTimeIntervalSinceNow:-4 * 3600 * 24];
//
//
//
//                    }else{
//                        weakself.IsHistory = NO;
//                        weakself.start_minDate = [NSDate dateWithTimeIntervalSinceNow:-3 * 3600 * 24];
//                        weakself.start_maxDate = [NSDate date];
//
//                        weakself.end_minDate = [NSDate dateWithTimeIntervalSinceNow:-3 * 3600 * 24];
//                        weakself.end_maxDate = [NSDate date];
//                    }
//
//                    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
...
//            }else if (type==2){
//#pragma mark-开始时间
//                MCDatePickerView * timePickView=[MCDatePickerView InitWithTitle:@"日期选择" Mindate:weakself.start_minDate MaxDate:weakself.start_maxDate];
//                timePickView.sureBlock = ^(NSString *selectDateStr) {
//                    weakself.popView.label3.text = [NSString stringWithFormat:@"%@ 00:00:00",selectDateStr];
//                    weakself.statTime = [NSString stringWithFormat:@"%@ 00:00:00",selectDateStr];
//                    weakself.end_minDate=[MCRecordTool getDateWithStr:[NSString stringWithFormat:@"%@ 00:00:00",selectDateStr]];
//                };
...
//#pragma mark-结束时间
//
//                MCDatePickerView * timePickView=[MCDatePickerView InitWithTitle:@"日期选择" Mindate:weakself.end_minDate MaxDate:weakself.end_maxDate];
//                timePickView.sureBlock = ^(NSString *selectDateStr) {
//                    weakself.start_maxDate=[MCRecordTool getDateWithStr:[NSString stringWithFormat:@"%@ 23:59:59",selectDateStr]];
//                    weakself.popView.label4.text = [NSString stringWithFormat:@"%@ 23:59:59",selectDateStr];
//                    weakself.endTime = [NSString stringWithFormat:@"%@ 23:59:59",selectDateStr];
//                };
//                [timePickView show];
