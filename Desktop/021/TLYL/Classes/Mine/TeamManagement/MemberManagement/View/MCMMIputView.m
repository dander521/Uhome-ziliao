...
@interface MCMMIputView()<UITableViewDelegate,UITableViewDataSource>

@property (nonatomic,weak) UITableView *tabView;

@end

@implementation MCMMIputView
...
- (void)show{
    [UIView animateWithDuration:0.1 animations:^{
        if (self.dataArray.count >=4) {
            self.frame = CGRectMake(0, G_SCREENHEIGHT -44 * 4 - 49, G_SCREENWIDTH, 44 * 4);
        } else {
            self.frame = CGRectMake(0, G_SCREENHEIGHT -44 * self.dataArray.count -49, G_SCREENWIDTH, 44 * self.dataArray.count);
        }
    }];
    self.tabView.frame = self.bounds;
